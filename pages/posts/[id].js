...
import postsPageInitialPathsAa3d0Resource from '../../resources/posts-page-initial-paths-aa3d0'
import postsPageInitialPropsAab57Resource from '../../resources/posts-page-initial-props-aab57'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Future Assurance Architect</title>
          <meta
            property="og:title"
            content="Posts1 - Future Assurance Architect"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsAa3d0Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsAab57Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
